  ...
  module Structure
    class Clone < Thor::Group
      ...
      def app_life_cycle_hooks_clone
        template 'app_life_cycle_hooks.rb.tt', "#{name}/features/support/app_life_cycle_hooks.rb" if android == 'true'
      end

      def app_installation_hook_clone
        template 'app_installation_hook.rb.tt', "#{name}/features/support/app_installation_hook.rb" if android == 'true'
      end

      def dry_run_clone
        template 'dry_run.rb.tt', "#{name}/features/support/dry_run.rb" if ios == 'true'
      end

      def first_launch_clone
        template 'first_launch.rb.tt', "#{name}/features/support/first_launch.rb" if ios == 'true'
      end
